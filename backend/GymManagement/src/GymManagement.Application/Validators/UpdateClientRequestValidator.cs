...
public class UpdateClientRequestValidator : AbstractValidator<UpdateClientRequest>
{
    public UpdateClientRequestValidator()
    {
        ...
        RuleFor(x => x.MembershipStartDate)
            .LessThan(x => x.MembershipEndDate).WithMessage("Membership start date must be before end date")
            .When(x => x.MembershipStartDate.HasValue && x.MembershipEndDate.HasValue);

        RuleFor(x => x.MembershipEndDate)
            .GreaterThan(x => x.MembershipStartDate).WithMessage("Membership end date must be after start date")
            .When(x => x.MembershipStartDate.HasValue && x.MembershipEndDate.HasValue);
